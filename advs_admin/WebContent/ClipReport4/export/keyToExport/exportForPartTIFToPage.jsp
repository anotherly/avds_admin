...
if(null != report_key){
	...
	//ExportInfo exportInfo = ClipReportExport.createExportForPartTIFToPage(request, report_key, tifOption, pageNumber);	
	
	//저장할 페이지 번호
	int pageNumber = 1;
	ExportInfo exportInfo = ClipReportExport.createExportForPartTIFToPage(request, report_key, tifOption, pageNumber, propertyPath);
	
	
	//아래의 기능은 멀티TIF 에서만 가능하기 때문에 강제로 멀티 TIF 옵션이 true 됍니다.
	//지정한 페이지를 배열에 설정하면 배열 순으로 TIF가 생성됩니다.
	//지정한 페이지가 잘못되었을 경우 TIF에 제외됩니다. (5페이지 리포트이지만 6페이지를 설정하였을 경우)
	//아래의 sample은 3페이지의 리포트를 페이지 역순으로 TIF 저장합니다.    
	//int[] pageNumList = new int[3];
    //pageNumList[0] = 3;   
    //pageNumList[1] = 2;
    //pageNumList[2] = 1;
	//ExportInfo exportInfo = ClipReportExport.createExportForPartTIFToPage(request, report_key, tifOption, pageNumList, propertyPath);
